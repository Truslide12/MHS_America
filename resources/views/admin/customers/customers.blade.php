...
<div class="row">
	...
	<div class="col-md-12">
		...
		<table class="table table-bordered table-hover table-striped dataTable" style="border-top:0;font-size: 1.5em;">
			<thead>
				<tr>
					<th class="salesteam" colspan="7">Company Accounts Index</th>
				</tr>
				<tr>
					<td>ID</td>
					<td>Customer</td>
					<td>Created</td>
					<td style="text-align: center;"><i class="fa fa-flag" title="Claimed"></i></td>
					<td style="text-align: center;"><i class="fa fa-credit-card" title="Paid"></i></td>
					<td style="text-align: center;"><i class="fa fa-certificate" title="Verified"></i></td>
				</tr>
			</thead>
			<tbody>
				@if( count($companies) <= 0 )
				<tr>
					<td colspan="6" style="text-align: center;">No new company accounts this week.</td>
				</tr>
				@else
				...
				<tr>
					...
					<td style="text-align: center;"><i class="fa @if($partner->claimed) fa-check @else fa-minus faded @endif"></i></td>
					<td style="text-align: center;"><i class="fa @if($partner->stripe_customer_id) fa-check @else fa-minus faded @endif"></i></td>
					<td style="text-align: center;"><i class="fa @if($partner->verified) fa-check @else fa-minus faded @endif"></i></td>
				</tr>
				@endforeach
				@endif
				<tr>
					<th class="salesteam" colspan="6">Personal Accounts Index</th>
				</tr>
				@if( count($pbas) <= 0 )
				<tr>
					<td colspan="6" style="text-align: center;">No new personal accounts this week.</td>
				</tr>
				@else
				@foreach($pbas as $partner)
				<tr>
					<td>{{$partner->id}}</td>
					<td><a href="{{ URL::route('admin-customer-edit', ['id'=> $partner->id]) }}">{{$partner->title}}</a></td>
					<td>{{$partner->created_at->format("m/d/Y")}}</td>
