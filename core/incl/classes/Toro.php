    ...
    public static function serve($routes)
    {
        ...
        if (!empty($_SERVER['PATH_INFO'])) {
            ...
            if (!empty($_SERVER['REQUEST_URI'])) {
                $path_info = (strpos($_SERVER['REQUEST_URI'], '?') > 0) ? substr($_SERVER['REQUEST_URI'], 0, strpos($_SERVER['REQUEST_URI'], '?')) : $_SERVER['REQUEST_URI'];
            }
        }
        ...
        if ($handler_instance) {
            ...
            if (method_exists($handler_instance, "route_".$request_method)) {
                ToroHook::fire('before_handler', compact('routes', 'discovered_handler', 'request_method', 'regex_matches'));
                $result = call_user_func_array(array($handler_instance, "route_".$request_method), $regex_matches);
