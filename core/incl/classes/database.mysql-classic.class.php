 ...
 *                                   *
 *  /inc/classes/database.class.php  *
 *************************************/
...
final class Database {
	...
	public function storeUser($obj) {
		...
		$result = $this->select_one()->from("users")->where("id='".$obj->user_id."'")->execute("fetchArray");
		if(!is_array($result)) {
	...
	public function getLogin() {
		if($this->_hasConnected != true){return array();}
		return $this->_db;
	}
	...
	public function insert_into($tabl) {
		...
		$this->_meth = "insert";
		$this->_curr = array();
		$this->_curr['insert'] = $this->_db['table_prefix'].$tabl;
		return self::$instance;
	}
	...
	public function update($tabl) {
		...
		$this->_meth = "update";
		$this->_curr = array();
		$this->_curr['update'] = $this->_db['table_prefix'].$tabl;
		return self::$instance;
	}
	
	public function set($flds, $vals, $insert = false) {
		if($this->_hasConnected != true){return self::$instance;}
		...
		}elseif($this->_meth == "update" && trim($this->_curr['update']) != "") {
			/* $this->_bumeth = $insert; */
			if(is_array($flds) && is_array($vals) && count($flds) == count($vals)){
				$this->_curr['fields'] = $flds;
				$this->_curr['values'] = $vals;
	...
	public function execute($res = "") {
		...
		if($this->_meth == "select" && trim($this->_curr['select']) != "" && trim($this->_curr['from']) != "") {
			...
			$offset = "";
			if(is_int($this->_curr['offset']) && (int) $this->_curr['offset'] >= 0){$offset = $this->_curr['offset'].", ";}
			if(is_int($this->_curr['limit']) && (int) $this->_curr['limit'] > 0){$query .= " LIMIT ".$offset.$this->_curr['limit'];}
			...
			if($res == "fetchArray"){
				...
				return $final_array;
			}elseif($res == "fetchCount") {
				return mysql_num_rows($ress);
			...
			if($this->_curr['where'] != ""){$query .= " WHERE ".$this->_curr['where'];}
			echo("<pre style=\"color:#fff;\">".$query."</pre>");
			$ress = mysql_query($query, $this->_handle);
			unset($query);
			$this->_curr = array();
			if(!$ress) {$return = false;}else{$return = true;}
