...
final class Database {
	...
	public function connect($dbdata) {
		...
		$this->_handle = @mysqli_connect($this->_db['host'], $this->_db['user'], $this->_db['pass'], $this->_db['database']);
		if($this->_handle === false){$this->throwError("FAILED TO CONNECT TO MYSQL SERVER");$this->_hasConnected = false;}else{
	...
	public function execute($res = "") {
		...
		if($this->_meth == "select" && trim($this->_curr['select']) != "" && trim($this->_curr['from']) != "") {
			...
			$ress = mysqli_query($this->_handle, $query);
			...
			if($res == "returnId") {
				if($return == true){return mysqli_insert_id($this->_handle);}else{return false;}
			}else{
				return $return;
			}
